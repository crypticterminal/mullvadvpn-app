...
- *Block when disconnected*: configures whether the state machine should apply the security policy
  ...
  system to never allow connections out of the tunnel
...
- *Block*: force the state machine to go to the `Error` state and block all connections
...
`enum` type representing which state the tunnel state machine it has entered and any associated
